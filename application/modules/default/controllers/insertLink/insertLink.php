...
function loadcontent($url, $cate)
{
    ...
    if(strpos($url,'raovat.vn')==11)
    {
        ...
        {
            if(checkLink("http://www.raovat.vn".$e->href)==0)
            {
                ...
                insertdb($url, "http://www.raovat.vn".$e->href, $date, 'raovat.vn', $cate, $content, 1, $title, $img, $city, $lienlac);
            }
            die();
        }
    }
...
<?php
insert();
/*$query = "CALL raovat_check_url_insert('http://www.raovat.vn/thanh-ly-36-chai-nuoc-hoa-ban-gia-goc-ben-usa-raovat-635508.html');";
		$link = mysql_connect('localhost', 'root', '') ;
        mysql_query('SET NAMES utf8',$link );
		mysql_select_db("raovatdb", $link);
		$result = mysql_query($query, $link);
